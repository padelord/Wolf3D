...
/*   Updated: 2020/02/12 20:29:38 by padelord         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		init_player(t_plr *plr, t_ivec2 orig, float angle, int *state)
{
...
int			new_image(t_img *t, void *mlx, t_ivec2 size, int *state)
{
	ft_bzero(t, sizeof(t_img));
	if (!(t->ptr = mlx_new_image(mlx, size.x, size.y)))
		return (R_IMG_INIT);
	t->w = size.x;
	t->h = size.y;
...
int		init_env(t_env *env, const char *path)
{
	...
	if ((env->win = mlx_new_window(env->mlx, WIDTH, HEIGHT, "Wolf3D")))
		...
		return (R_WIN_INIT);
	if ((err = new_image(env->img, env->mlx, (t_ivec2){WIDTH, HEIGHT}, &env->state)))
		return (err);
	if ((err = get_map(path, env->map, &env->state)))
		return (err);
	if ((err = init_player(env->plr, env->map->orig, 0.0, &env->state)))
		return (err);
